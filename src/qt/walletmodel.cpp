...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered faqo address / amount:
...

    {
        ...
        if (recipients[0].useHyperSend && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("HyperSend doesn't support sending values that high yet. Transactions are currently limited to %1 FFQ.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal faqo:URI (faqo:XyZ...?message=example)
            {
...
static void NotifyzFFQReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzFFQReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzFFQReset.connect(boost::bind(NotifyzFFQReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzFFQReset.disconnect(boost::bind(NotifyzFFQReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zFFQTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
