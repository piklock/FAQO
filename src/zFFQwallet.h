...
#ifndef FAQO_ZFFQWALLET_H
#define FAQO_ZFFQWALLET_H
...
public:
    CzFFQWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZFFQ(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZFFQ(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //FAQO_ZFFQWALLET_H
