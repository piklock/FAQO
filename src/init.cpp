...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "faqo.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "faqod.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the FFQ and zFFQ money supply statistics") + " " + _("on startup"));
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 55581, 55681));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in FFQ/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in FFQ/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, faqo, (obfuscation, hypersend, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in FFQ/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all FAQO specific functionality (Masternodes, Zerocoin, HyperSend, Budgeting) (0-1, default: %u)"), 0));

#ifdef ENABLE_WALLET
    strUsage += HelpMessageGroup(_("Staking options:"));
    strUsage += HelpMessageOpt("-staking=<n>", strprintf(_("Enable staking functionality (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-FFQstake=<n>", strprintf(_("Enable or disable staking functionality for FFQ inputs (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zFFQstake=<n>", strprintf(_("Enable or disable staking functionality for zFFQ inputs (0-1, default: %u)"), 1));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:55581"));
    ...
    strUsage += HelpMessageOpt("-backupzFFQ=<n>", strprintf(_("Enable automatic wallet backups triggered after each zFFQ minting (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zFFQbackuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic zFFQ backup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as well, 4 backups will happen"));
#endif // ENABLE_WALLET
...
//    strUsage += "  -anonymizefaqoamount=<n>     " + strprintf(_("Keep N FFQ anonymized (default: %u)"), 0) + "\n";
//    strUsage += "  -liquidityprovider=<n>       " + strprintf(_("Provide liquidity to Obfuscation by infrequently mixing coins on a continual basis (0-100, default: %u, 1=very frequent, high fees, 100=very infrequent, low fees)"), 0) + "\n";
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 55081, 51481));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2015-%i The FAQO Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that FAQO is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. FAQO Core is shutting down."));
...
#endif
    // Make sure only a single FAQO process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. FAQO Core is probably already running."), strDataDir));
...
// ********************************************************* Step 5: Backup wallet and verify wallet database integrity
    ...
    //FAQO: Load Accumulator Checkpoints according to network (main/test/regtest)
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                //FAQO specific: zerocoin and spork DB's
                ...
                // FAQO: load previous sessions sporks if we have them.
                ...
                if (GetBoolArg("-reindexmoneysupply", false)) {
                    if (chainActive.Height() > Params().Zerocoin_StartHeight()) {
                        RecalculateZFFQMinted();
                        RecalculateZFFQSpent();
                    }
                    RecalculateFFQSupply(1);
                }
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of FAQO Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart FAQO Core to complete") << "\n";
        ...
        zwalletMain = new CzFFQWallet(pwalletMain->strWalletFile);
        ...
        //Inititalize zFFQWallet
        uiInterface.InitMessage(_("Syncing zFFQ wallet..."));

        bool fEnableZFfqBackups = GetBoolArg("-backupzFFQ", true);
        pwalletMain->setZFfqAutoBackups(fEnableZFfqBackups);

        //Load zerocoin mint hashes to memory
        pwalletMain->zFFQTracker->Init();
...
//    }
//
//    nAnonymizeFAQOAmount = GetArg("-anonymizefaqoamount", 0);
//    if (nAnonymizeFAQOAmount > 999999) nAnonymizeFAQOAmount = 999999;
//    if (nAnonymizeFAQOAmount < 2) nAnonymizeFAQOAmount = 2;
    ...
    LogPrintf("Anonymize FAQO Amount %d\n", nAnonymizeFAQOAmount);
    ...
    /* Denominations
       ...
       For example:
       1FFQ+1000 == (.1FFQ+100)*10
       10FFQ+10000 == (1FFQ+1000)*10
    */
